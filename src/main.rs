...
fn main() {
    App::new()
        ...
        .add_systems(Startup, (spawn_reactor, spawn_reactor_boxes))
        .add_systems(Update, keyboard_move)
...
fn setup(
    ...
    commands.spawn((Camera2dBundle {
        transform: Transform::from_translation(Vec3::new(0.0, 0.0, 100.0)).looking_at(Vec3::ZERO, Vec3::Y),
...
fn spawn_reactor(
    ...
    let reactor_offset = Vec3::new(-((window_size.0 as f32)/3.0 * 0.9) , (window_size.1 as f32)/6.0, 0.0);
    for (mut amount, offset, spacing, exclude) in sns {
        ...
        for x_coord in iter.step_by(spacing as usize) {
            ...
            commands.spawn(
                SpriteBundle {
                    sprite: square.clone(),
                    transform: Transform::from_translation(reactor_offset + Vec3::new(pos.x, pos.y, 0.0)),
                    ..Default::default()
                }
            );
            commands.spawn((
                SpriteBundle {
                    ...
                    transform: Transform::from_translation(reactor_offset + Vec3::new(pos.x, pos.y, 0.1)),
                    ..Default::default()
                },
                Rod::new(
                    pos,
                    rod_number, 
                    "startup neutron source".to_string()),
            ));
            ...
            if !(exclude.is_some() && exclude.clone().unwrap().contains(&x_coord)) {
                ...
                commands.spawn((
                    SpriteBundle {
                        ...
                        transform: Transform::from_translation(reactor_offset + Vec3::new(pos.x, pos.y, 1.0)),
                        ..Default::default()
                    },
                    Rod::new(
                        pos,
                        rod_number, 
                        "automatic control rod".to_string()),
                ));
            }
            fr_exclude.push(Vec2::new(x_coord as f32, y_counter as f32));
...
fn spawn_reactor_boxes(
    ...
    let window_size = Vec2::new(window.single().physical_width() as f32, window.single().physical_height() as f32);
    let reactor_offset = Vec3::new( -(window_size.x/3.0 * 0.9), -(window_size.y /3.0), 0.0);
    ...
    commands.spawn(Text2dBundle{
        ...
        transform: Transform::from_translation(reactor_offset+Vec3::new(-tenth.x, tenth.y+50.0, 0.0)),
        ..Default::default()

    });
    commands.spawn(SpriteBundle {
        sprite: rect.clone(),
        transform: Transform::from_translation(reactor_offset+Vec3::new(-tenth.x, tenth.y, 0.0)),
        ..Default::default()
    });
    commands.spawn(SpriteBundle {
